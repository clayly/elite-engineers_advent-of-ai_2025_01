...
## Агент исправления кода (agent_codefix.py)

Этот асинхронный агент на базе LangChain сканирует Python‑проект, находит синтаксические и импорт‑ошибки, предлагает исправления через LLM (OpenRouter) и по вашему подтверждению применяет патчи.

Требования:
- Python 3.13+
- Переменная окружения `OPENROUTER_API_KEY` (допускается запасной `OPENAI_API_KEY`)

Пример запуска:
```
# Весь проект в текущей директории
python agent_codefix.py --path .

# Для одиночного файла
python agent_codefix.py --path src/module.py

# С автоматическим применением правок без вопросов
python agent_codefix.py --path . --apply --yes

# Настройка модели и параметров генерации
python agent_codefix.py --path . --model openai/gpt-4o-mini `--temperature 0.0 --max-tokens 1200`
```

Поведение:
- Агент рекурсивно собирает .py файлы (пропуская служебные каталоги вроде .git, __pycache__, .venv и т.п.).
- Для каждого файла пытается:
  - разобрать AST (`ast.parse`) и зафиксировать SyntaxError с контекстной строкой;
  - проанализировать `import` / `from ... import ...` и проверить разрешимость модулей в пределах проекта и через `importlib.util.find_spec` c добавлением корня проекта в `sys.path` (без выполнения кода проекта).
- Для файлов с проблемами отправляет краткий контекст и список ошибок в LLM через LangChain (`ChatOpenAI`) с базовым URL OpenRouter `https://openrouter.ai/api/v1` и рекомендуемыми заголовками (`HTTP-Referer`, `X-Title` если заданы `OPENROUTER_SITE_URL`, `OPENROUTER_APP_NAME`).
- Полученный фикс возвращается целиком в виде Python‑кода; агент может показать предпросмотр и предложить применить изменения. При применении создаётся `.bak`‑резервная копия исходника.
